...
### Requirement: Обновление подключённого репозитория контекста
Система SHALL позволять пользователю переключать сохранённый репозиторий контекста на существующую local либо remote-tracking ветку и получать обновления текущей ветки только fast-forward способом, сохраняя AI-доступ к репозиторию read-only.

#### Scenario: Выбор локальной ветки
- **WHEN** пользователь выбирает существующую local branch чистого подключённого репозитория
- **THEN** backend переключает worktree на точную выбранную ветку и возвращает перечитанное состояние репозитория

#### Scenario: Выбор remote-tracking ветки
- **WHEN** пользователь выбирает существующую remote-tracking branch, для которой нет одноимённой local branch
- **THEN** backend создаёт tracking branch с проверенным локальным именем, переключает worktree и возвращает обновлённое состояние

#### Scenario: Получение обновлений
- **WHEN** пользователь запускает обновление чистой ветки с upstream
- **THEN** backend выполняет неинтерактивный fetch и `pull --ff-only`, затем возвращает актуальные HEAD, branch, divergence и список веток

#### Scenario: Локальные изменения
- **WHEN** подключённый репозиторий содержит staged, unstaged или untracked изменения
- **THEN** switch и получение обновлений отклоняются с `WORKTREE_DIRTY` без stash, reset или discard

#### Scenario: Невозможный fast-forward
- **WHEN** текущую ветку нельзя обновить fast-forward способом
- **THEN** backend возвращает `GIT_FAST_FORWARD_REQUIRED`, не выполняя merge или rebase
