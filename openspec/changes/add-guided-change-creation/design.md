...
### 14. Итоговый Markdown получает полноэкранный feedback loop

Каждая create/update/rename-мутация широкого review предоставляет действие открытия итогового Markdown. Вложенный полноэкранный dialog показывает read-only Markdown и независимую review-колонку, где пользователь может добавить, отредактировать или удалить несколько замечаний. Замечания принадлежат текущей клиентской review-сессии, ограничиваются по количеству и длине и не записываются в артефакт.

Действие «Повторить этап» доступно для artifact-операций `prepare_artifact` и `fix_artifact`: controller сначала отклоняет текущий изолированный result set, перечитывает актуальный fingerprint change, затем запускает тот же action/change/artifact с целью, содержащей bounded review feedback. Старый result set не принимается автоматически, а новая операция проходит полный цикл running → awaiting_review. «Принять исправления» использует существующий accept текущей операции и не объединяет мутации разных итераций.

Полноэкранный dialog имеет собственный sticky header/footer, не зависит от прокрутки split diff и закрывается без потери текущей operation. После запуска повторной итерации локальные замечания очищаются только когда backend подтвердил старт новой операции.
...
- [Большой feedback раздувает action goal] → ограничить число, длину одного замечания и суммарный размер перед повторным запуском.
