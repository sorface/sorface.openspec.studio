...
- Постоянный лаконичный selector текущей задачи/ветки с семантической branch-иконкой перед именем в верхней панели workspace.
...
- Единое icon-only действие «Опубликовать» в центральной группе статусов header для OpenSpec-изменений текущей задачи без stage, commit message, fetch, upstream и push controls.
- Немедленное открытие publication dialog с редактируемым сообщением и без ожидания agent CLI.
- Опциональная генерация русского commit message выбранным локальным agent CLI из точного ограниченного diff в формате `<branch>: <короткое сообщение>` и списка изменений.
- Автоматический обычный push ветки задачи без force и с безопасной обработкой remote divergence.
- Компактное подтверждение публикации с перечнем артефактов и редактируемым сообщением.
...
- Публикация сразу формирует точный набор изменений и редактируемое сообщение; по явному запросу пользователя agent предлагает русский subject/body, после чего backend повторно проверяет diff и выполняет commit/push.
...
- `git-delivery`: ручной Git workflow заменяется публикацией OpenSpec-артефактов текущей задачи с пользовательским либо опционально сгенерированным commit message и обычным push.
...
- Тесты: backend service/API, frontend controller/components, интеграционные сценарии dirty task switching, ручного сообщения, опциональной agent-генерации и browser screenshots.
...
- Agent может вернуть некорректное или выдуманное сообщение; результат проверяется структурно, не заменяет пользовательский текст при ошибке и не блокирует ручную публикацию.
